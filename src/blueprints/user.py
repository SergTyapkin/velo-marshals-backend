...
def userUpdate(userData):
    ...
    if avatarUrl is None and 'avatarUrl' not in req: avatarUrl = userData['avatarUrl']

    try:
        resp = DB.execute(sql.updateUserById, [firstName, secondName, thirdName, email, telegram, title, avatarUrl, userId])
    except:
        return jsonResponse("Имя пользователя или email заняты", HTTP_DATA_CONFLICT)
...
def userRestorePasswordSendEmail():
    ...
    send_email(email,
               ...
               emails.restorePassword(f"/image/{userData['avatarUrl']}", userData['firstname'] + ' ' + userData['secondname'], secretCode))
...
def userAuthByEmailCode():
    ...
    if code is None:
        ...
        send_email(email,
                   "Вход на TechSupport",
                   emails.loginByCode(f"/image/{avatarUrl}" if avatarUrl is not None else None,
...
def userConfirmEmailSendMessage(userData):
    ...
    send_email(email,
               ...
               emails.confirmEmail(f"/image/{userData['avatarUrl']}", userData['firstname'] + ' ' + userData['secondname'], secretCode))
